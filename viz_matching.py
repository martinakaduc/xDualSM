...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--source", help="source graph idx", type=int, default=0
    )
    parser.add_argument(
        "--query", help="query graph idx", type=int, default=0
    )
    parser.add_argument(
        "--synthesis", action="store_true", help="synthesis data"
    )
    ...
    graphs = utils.read_graphs(
        f"{data_path}/{args.source}/source.lg")
    graph = graphs[0]
    ...
    if True:
        ...
        for node in subgraph.nodes:
            cnode_mapping = list(map(lambda y: (y[1], interaction_dict[y]), filter(
                lambda x: x[0] == node, list_mapping)))
            if len(cnode_mapping) == 0:
                mapping_dict[node] = []
                continue

            max_prob = max(cnode_mapping, key=lambda x: x[1])[1]
            mapping_dict[node] = list(map(lambda x: x[0], filter(
