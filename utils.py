...
def plotGraph(graph: nx.Graph, nodeLabels=None, showLabel=True, nodeColors=None, edgeColors=None):
    pos = nx.circular_layout(graph)
    plt.figure()

    if not nodeLabels:
        nodeLabels = {node:node for node in graph.nodes} if not showLabel \
                else {node:(node, graph.nodes[node]["label"]) \
    ...
    nx.draw(graph, pos, edge_color=edgeColors, width=1, linewidths=0.1,
